...
- **Never create pull requests.** No `gh pr create`, no `mcp__github__create_pull_request`, no PR-shaped workflows of any kind.
- **If direct push to `main` is blocked**, push the commit to a throwaway branch (e.g. `tmp-<short-desc>`) and stop. Tell the user the branch name; they will land it. Once it's on `main`, delete the throwaway branch locally and remotely.
...
- **Never add attribution to commit messages, branch names, code comments, or generated docs** — no "Co-Authored-By", no "Generated by Claude Code", no `claude.ai/code` links, no "🤖" sigils, no AI tool fingerprints of any kind. Legitimate technical references to the product's stack (e.g. "Claude Agent SDK" in architecture docs) are allowed; self-attribution by the assistant is not.
